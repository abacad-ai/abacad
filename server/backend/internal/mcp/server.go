...
func dispatch(ctx context.Context, req request, resolver DeviceResolver, scope Scope, blobs BlobStore, grants ComputerGrantIssuer) *response {
	switch req.Method {
	case "initialize":
	...
	case "tools/list":
		...
		return &resp
	case "tools/call":
		resp := resultResponse(req.ID, callTool(ctx, req.Params, resolver, scope, blobs, grants))
		return &resp
	default:
...
func callTool(ctx context.Context, params json.RawMessage, resolver DeviceResolver, scope Scope, blobs BlobStore, grants ComputerGrantIssuer) toolResult {
	var p struct {
	...
	if tool.computerCall != nil {
		return tool.computerCall(ctx, dc, p.Arguments, resolver.AccountID(), grants)
	}
